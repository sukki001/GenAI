...
///tool to execute any terminal/shell command
...
async function executeCommand({command}) {
    ...
    try{
        ...
        return `Success : ${stdout} || Task Executed sucessfully.`
    }
    catch(error){
        return `ERROR : ${error}`
    }
...
const executeCommandDeclarataion = {
    name : 'executeCommand',
    description : 'Execurte a single terminal/Shell command. A command can be to create a folder , file , write on a file , edit the file or delete the file',
    parameters : {
        type : 'OBJECT',
        properties : {
            command : {
        ...
        const response = await ai.models.generateContent({
            ...
            config: {
                ...
                **ABSOLUTE RULE:** Do not use a single \`echo "long string of code..." > file.html\` command for writing complex files. It is unreliable. Always use the specific multi-line methods described above for each OS.
                ...
                ` ,
                tools: [{
                    functionDeclarations: [executeCommandDeclarataion]
                }],
            },
        }); 
...
async function main() {
    console.log("I'm a cursor AI, Let's build a website.");
